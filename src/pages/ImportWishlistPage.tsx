...
const ImportWishlistPage = () => {
  ...
  useEffect(() => {
    ...
    if (user && shareId && !authLoading && !importMutation.isPending) {
  ...
  }, [user, shareId, authLoading, navigate, importMutation]); // Note: Removed importMutation.mutate from deps to avoid infinite loop

  // Show a loading state while authentication or import is in progress
  if (authLoading || importMutation.isPending) {
    return (
      <Layout>
